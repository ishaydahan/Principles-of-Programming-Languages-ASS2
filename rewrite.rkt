  ...
  (lambda (exp)
    (cond ((def-exp? exp) (make-def-exp (def-exp->var exp)
          ...
          ((if-exp? exp) (make-if-exp
                          ...
                          (rewrite-all-let* (if-exp->then exp))
                          (rewrite-all-let* (if-exp->else exp))))
          ((let-exp? exp) (make-let-exp (map rewrite-all-let* (let*-exp->bindings exp))
                                        (map rewrite-all-let* (let*-exp->body exp))))
          ((binding? exp) (make-binding
                           (binding->var exp)
                           (rewrite-all-let* (binding->val exp))))
          ((let*-exp? exp) (rewrite-all-let* (letrec(
                                                     (bindings (let*-exp->bindings exp))
                                                     (body (let*-exp->body exp))
                                                     (rewrite-let* (lambda (n) (if (empty? bindings) (make-let-exp bindings body)
                                                                                   (make-let-exp (list (list-ref bindings n))
                                                                                                 (if (= (+ n 1) (length bindings))
                                                                                                     body
                                                                                                     (list(rewrite-let* (+ 1 n)))))))))
                                               (rewrite-let* 0))))
          ((app-exp? exp) (make-app-exp (rewrite-all-let* (app-exp->rator exp))
                                        (map rewrite-all-let* (app-exp->rands exp))))
          (else (error "Unknown exp type: " exp)))))
...
;(eval
;(unparse(rewrite-all-let* (parse '(if (let* ((x 4) (y x)) (+ y x) #t)
;(let* ((x 4) (y x)) (+ y x) (display 6)) (let* ((x 4) (y x)) (+ y x) (display 7))))))
;ns)